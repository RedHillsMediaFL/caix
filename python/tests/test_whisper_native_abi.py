...
def test_large_v2_abi_uses_the_explicit_load_once_cross_kv_bridge() -> None:
    ...
    assert abi.schema == "caix.whisper-split.v2"
    ...
    assert abi.tensors == {
        ...
        "load_status": abi_module.TensorABI("int32", (1,)),
        "logits": abi_module.TensorABI("float16", (1, 1, 51865)),
        "decode_status": abi_module.TensorABI("int32", (1,)),
    }
    assert abi.success_status == 1
    assert abi.invalid_state_status == 0

...
- Recorded the authored Ornith-1.0-35B int4+head-quant path: one-layer and four-layer structural bundles pass GPU fast-path smoke, and the full 40-layer export completes at 17 GB, but full runtime warmup currently fails with an MPS reshape error, so HF publication remains held.
